	...
	var MgekoAdvancedSearchForm = class extends AdvancedSearchForm {
		...
		getSections() {
			if (this.isQuerySearch) return [Section("info", [LabelRow("queryFilteringUnsupported", {
				...
				subtitle: "Filtering is not supported when searching by title. Clear the search text to use filters."
			})])];
			return [
...
//#endregion
//#region src/Mgeko/models.ts
	const DOMAIN = "https://www.mgeko.cc";

//#endregion
//#region src/Mgeko/network.ts
	var MgekoInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#endregion
//#region src/Mgeko/parsers.ts
	const parseMangaDetails = ($, mangaId, sourceUrl) => {
	...
	const parseChapters = ($, sourceManga) => {
		const chapters = [];
		...
		if (chapters.length == 0) throw new Error(`Couldn't find any chapters for mangaId: ${sourceManga.mangaId}!`);
		return chapters;
	};
	const parseChapterDetails = ($, chapter) => {
		const pages = [];
...
//#region src/Mgeko/main.ts
	var MgekoExtension = class {
		constructor() {
			...
			_defineProperty(this, "mainRequestInterceptor", new MgekoInterceptor("main"));
		...
		async getDiscoverSectionItems(section, metadata) {
			switch (section.id) {
				case "popular_all_time": return this.getFilteredSectionItems("popular_all_time", metadata);
				case "top_rated": return this.getFilteredSectionItems("rating", metadata);
				case "latest_updates": return this.getFilteredSectionItems("latest", metadata);
				case "genres": return this.getGenreSectionItems();
				default: return {
					items: [],
					metadata: void 0
				};
		...
		async getGenreTags() {
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("browse-comics").toString(),
				method: "GET"
			};
			return parseGenreTags(await this.fetchCheerio(request));
		}
		async getMangaDetails(mangaId) {
			...
			return parseMangaDetails(await this.fetchCheerio(request), mangaId, DOMAIN);
		}
		async getChapters(sourceManga) {
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("manga").addPathComponent(sourceManga.mangaId).addPathComponent("all-chapters").toString(),
				method: "GET"
			};
			return parseChapters(await this.fetchCheerio(request), sourceManga);
		}
		async getChapterDetails(chapter) {
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("reader").addPathComponent("en").addPathComponent(chapter.chapterId).toString(),
				method: "GET"
			};
			return parseChapterDetails(await this.fetchCheerio(request), chapter);
		}
		async getSortingOptions() {
			return [
				...
				{
					id: "popular_weekly",
					label: "Popular Weekly"
				},
				{
					id: "popular_monthly",
					label: "Popular Monthly"
				},
				{
					id: "popular_all_time",
		...
		async getSearchResults(query, metadata, sortingOption) {
			...
			if (query.title?.trim().length ?? false) {
				const request = {
					url: new URL$1(DOMAIN).addPathComponent("autocomplete").setQueryItem("term", query.title.trim()).toString(),
					method: "GET"
				};
				return { items: parseOldSearch(await this.fetchCheerio(request), DOMAIN) };
			} else {
				const urlBuilder = new URL$1(DOMAIN).addPathComponent("browse-comics").addPathComponent("data");
				...
				const manga = parseSearch(load(parsedData.results_html, { xml: {
					xmlMode: false,
					decodeEntities: false
				} }), DOMAIN);
				metadata = parsedData.page < parsedData.num_pages ? { page: page + 1 } : void 0;
				return {
					items: manga,
					metadata
				};
		...
		async getFilteredSectionItems(sort, metadata) {
			...
			const parsedData = await this.fetchApi(request);
			const manga = parseViewMore(load(parsedData.results_html, { xml: {
				xmlMode: false,
				decodeEntities: false
			} }));
			metadata = parsedData.page < parsedData.num_pages ? { page: page + 1 } : void 0;
		...
		async getGenreSectionItems() {
			return {
				items: (await this.getGenreTags())[0].tags.map((genre) => ({
		...
		async fetchCheerio(request) {
			...
			return load(Application.arrayBufferToUTF8String(data), { xml: {
				xmlMode: false,
				decodeEntities: false
			} });
		}
...
//#endregion
//#region src/tests/Mgeko.ts
	async function runTests(logger) {
		const suite = new TestSuite("Mgeko tests", logger);
		registerDefaultTests(suite, Mgeko, pbconfig_default);
		await suite.run();
	}
